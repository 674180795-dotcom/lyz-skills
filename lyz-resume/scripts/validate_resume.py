...
def validate_data(data: dict[str, Any]) -> tuple[list[str], list[str], dict[str, int]]:
    ...
    if photo.get("enabled") is True:
        ...
        if not source:
            errors.append("照片已启用，但 basics.photo.source 为空")
        elif source.startswith(("http://", "https://", "data:")):
            errors.append("照片必须是本地文件，不能使用远程 URL 或预置 data URI")
        elif Path(source).suffix.lower() not in {".jpg", ".jpeg", ".png", ".webp"}:
            errors.append("照片仅支持 JPG、PNG 或 WebP")
    ...
    if counts["education"] == 0:
        warnings.append("教育经历为空；仅在用户明确省略或确实无法提供时保留")
...
def inspect_html(
    ...
    *,
    ...
    if re.search(r'<img[^>]+src=["\']https?://', text, flags=re.IGNORECASE):
        errors.append("HTML 图片不应依赖远程 URL")
    facts["theme"] = theme
...
def inspect_pdf(pdf: Path, expected: list[str], *, theme: str, require_theme_font: bool) -> tuple[list[str], list[str], dict[str, Any]]:
    ...
    if pdftotext:
        ...
        try:
            ...
            facts["extracted_characters"] = len(extracted)
            facts.setdefault("pages", len(reader.pages))
            ...
            warnings.append("未找到 pdftotext 或 pypdf；文本可提取性为 missing evidence")

    pdffonts = find_pdf_tool("pdffonts")
    if pdffonts:
...
def main() -> None:
    ...
    if args.pdf:
        ...
        if data.get("experience"):
            section_word = "工作经历" if language == "zh-CN" else "EXPERIENCE"
        elif data.get("projects"):
            section_word = "项目经历" if language == "zh-CN" else "PROJECTS"
        else:
