...
THEMES: dict[str, dict[str, Any]] = {
    ...
    "campus": {
        "label": "轻盈清新",
        "filename": "05_轻盈清新",
        "use_case": "第一份实习、运营、市场、教育与早期职业求职",
        "page": "#FFFDF8",
        "paper": "#FFFDF8",
        "ink": "#17231F",
        "muted": "#64706B",
...
def hydrate_photo(data: dict[str, Any], base_dir: Path) -> None:
    """Embed an explicitly enabled local portrait so HTML has no path dependency."""
    ...
    if not source:
        raise ValueError("照片已启用，但 basics.photo.source 为空")
    if source.startswith(("http://", "https://", "data:")):
        raise ValueError("照片必须是用户提供的本地文件，不能使用远程 URL 或预置 data URI")
    photo_path = Path(source).expanduser()
    if not photo_path.is_absolute():
        photo_path = base_dir / photo_path
    photo_path = photo_path.resolve()
    if not photo_path.is_file():
        raise ValueError(f"照片文件不存在：{photo_path}")
...
def theme_css(theme_id: str, reference_style: str | None = None) -> str:
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .portrait {{ width: 21mm; height: 27mm; flex: 0 0 auto; object-fit: cover; object-position: center; }}
    ...
    photo_css = """
      .identity.has-photo { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, auto) 21mm; align-items: center; }
      .identity.has-photo .position { min-width: 0; }
      .identity.has-photo .portrait { grid-column: 3; }
      .theme-ats-classic .identity.has-photo { text-align: left; }
      .theme-ats-classic .identity.has-photo .position { text-align: right; margin-top: 0; }
      .theme-ats-classic .identity.has-photo .contact { justify-content: flex-end; }
      .theme-swiss .identity.has-photo { display: grid; }
      .theme-swiss .identity.has-photo .position { display: block; margin-top: 0; text-align: right; }
    """
    return common + variants[theme_id] + reference_css + photo_css
...
<head>
  ...
  <meta name="generator" content="Resume Studio · local single-source renderer">
...
<main>
  <header>
    <div class="identity{' has-photo' if portrait else ''}">
      <h1>{name}</h1>
      <div class="position">
...
def print_pdf(browser: str, html_path: Path, pdf_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="resume-studio-chrome-") as profile:
        command = [
            browser,
            "--headless=new",
            "--disable-gpu",
